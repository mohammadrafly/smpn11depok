...
<div class="bg-white rounded-lg shadow-lg w-full p-5">
    <div class="flex justify-between items-center mb-5">
        ...
        <a href="{{ route('artikel.create') }}" class="bg-blue-500 text-white px-4 py-2 ml-3 rounded-lg shadow-lg hover:bg-blue-600 transition-colors duration-300">Tambah</a>
    </div>
...
<script>
            ...
            const row = `
                        <tr>
                            ...
                            <td class="p-3">
                                ...
                                <button onclick="deleteArticle(${item.id})" class="bg-red-500 rounded-lg p-2 text-white font-semibold">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
    ...
    function editArticle(id) {
        const baseUrl = "{{ route('artikel.update', ['id' => '__ID__']) }}";
        window.location.href = baseUrl.replace('__ID__', id);
    }

    function deleteArticle(id) {
        const baseUrl = "{{ route('artikel.delete', ['id' => '__ID__']) }}";
