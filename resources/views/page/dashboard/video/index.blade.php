...
<div class="bg-white rounded-lg shadow-lg w-full p-5">
    <div class="flex justify-between items-center mb-5">
        ...
        <a href="{{ route('video.create') }}" class="bg-blue-500 text-white px-4 py-2 ml-3 rounded-lg shadow-lg hover:bg-blue-600 transition-colors duration-300">Tambah</a>
    </div>
...
<script>
    ...
    function editData(id) {
        const baseUrl = "{{ route('video.update', ['id' => '__ID__']) }}";
        window.location.href = baseUrl.replace('__ID__', id);
    }

    function deleteData(id) {
        const baseUrl = "{{ route('video.delete', ['id' => '__ID__']) }}";
